        ...
        while not converged:
            ...
            for set in viableSets:
                ...
                if not set.explored():
                    ...
                    candidateSet = random.multivariate_normal([0] * self.model.nParams,
                                                              np.diag(set.pca.explained_variance_) * set.varScale,
                                                              self.nsamples)
                    ...
                    pickle.dump(candidateSet,
                                open(filename + "_Region" + str(set.label) + "CandidateSet_Iter" + str(set.iter) + ".p",
                                     "wb+"))
                    pickle.dump(viablePoints,
                                open(filename + "_Region" + str(set.label) + "ViableSet_Iter" + str(set.iter) + ".p",
                                     "wb+"))
                ...
                if set.cluster and (maxDepth == 0 or set.depth < maxDepth):
                    ...
                    if k > 1:
                        ...
                        for i in range(k):
                            ...
                            if len(points) > len(set.components):
